package com.bergburg.bergburgdelivery.Constantes;

public class Constantes {
    public static String SIM = "SIM";
    public static String NAO = "NAO";
    public static String LOGADO = "Logado";
    public static String MENSAGEM = "Mensagem";
    public static String DESLOGADO = "Deslogado";
    public static String ENVIADO = "Envidado";
    public static String ID_CATEGORIA = "idCategoria";
    public static String ID_PRODUTO = "idProduto";
    public static String ID_PEDIDO = "idPedido";
    public static String ID_USUARIO = "idUsuario";
    public static String ID_CONVERSA = "idConversa";
    public static String NOME = "nome";
    public static String SEM_INTERNET = "Sem internet";
    public static String INSTABILIDADE = "Instabilidade na conexão";
    public static String ALTERADO_SUCESSO = "Alterado com sucesso";
    public static String LOGIN_SUCESSO = "login com sucesso";
    public static String CADASTRO_SUCESSO = "Conta cadastrada com sucesso";
    public static String EMAIL_ENVIADO = "E-mail enviado";
    public static String EMAIL_NAO_CADASTRADO = "E-mail não cadastrado";
    public static Double LATITUDE_BERGS_BURG = -1.3767920885335923;
    public static Double LONGITUDE_BERGS_BURG = -48.436112643121895;
    public static Long ADMIN = 1L;

}
